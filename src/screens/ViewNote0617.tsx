...
const ViewNote0617 =  ({route}:any) => {
  ...
  for (let k =0; k< noteArray.length;k++){
    //한 이미지에 대한 내용들
    let [noteOriginW,noteOriginH] = noteArray[k][0];
    let pitches = noteArray[k][2];
    const pitchTextArrPerImg :any[] = [];
    for (let i=0; i<pitches.length; i++){
      ...
      pitchTextArrPerImg.push(
        <View style={{height:maxLen*17,backgroundColor: i%2==0? 'white':'#E7E7E7'}}>
        {pitchTextArrPerLine}
        </View>
        )
    }
    pitchTextArr.push(pitchTextArrPerImg)
  }
  
  for (let i=0; i<imgArray.length; i++){
    let imgObjArrPerImg: any[]=[];
    for (let j=0; i<imgArray[i].length; j++){
      imgObjArrPerImg.push(
        <Image
          key={i}
    ...
    imgObjArr.push(imgObjArrPerImg)
  }
  ...
  const Note = ()=>{
    ...
    for (let i=0; i<pitchTextArr.length;i++){
      for (let j=0; j<pitchTextArr[i].length;j++){
        finalObjs.push(
          imgObjArr[i][j],
          pitchTextArr[i][j]
        )
